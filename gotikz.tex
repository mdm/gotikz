...
\def\gotikz@marks@triangle{0}
\def\gotikz@marks@square{1}
\def\gotikz@marks@circle{2}
...
\def\black#1{%
    \gotikz@coordinates@addtolist\gotikz@currentdiagram@blackstones{#1}
}

% \def\mylist{\gotikz@currentdiagram@marks@coordinates}

\def\white#1{%
    \gotikz@coordinates@addtolist\gotikz@currentdiagram@whitestones{#1}
}

\pgfkeys{%
    /gotikz/mark/@selected/.initial=\gotikz@marks@triangle,
...
\def\gotikz@addmarks@withoptions[#1]#2{%
    \wlog{WITHOPTIONS: (#1)(#2)}
    \wlog{\pgfkeysvalueof{/gotikz/mark/@selected},\gotikz@marks@selected}
    \pgfkeys{/gotikz/mark/.cd,#1}{%
...
\def\gotikz@addmarks@chooseaction{%
    \ifx\gotikz@addmarks@nexttoken[%
        \wlog{CHOOSEACTION: YES}
        \expandafter\gotikz@addmarks@withoptions
    \else
        \wlog{CHOOSEACTION: NO}
        \expandafter\gotikz@addmarks@nooptions
    \fi
}

\def\addmarks{\futurelet\gotikz@addmarks@nexttoken\gotikz@addmarks@chooseaction}
            ...
            \foreach \x/\y [count=\c,evaluate=\c as \type using {{\gotikz@currentdiagram@marks@values}[\c-1]}] in \gotikz@currentdiagram@marks@coordinates {%
                \gotikz@coordinates@contains{\gotikz@currentdiagram@blackstones}{\x/\y}\iftrue
                    ...
                    \ifx\type\gotikz@marks@circle
                        ...
                        \draw [fill = black, draw = white] (\x,\y) circle (0.28cm);
                    \fi
                \else
                    \ifx\type\gotikz@marks@triangle
                    ...
                    \fi
                    \ifx\type\gotikz@marks@circle
                        \draw [fill = white, draw = black] (\x,\y) circle (0.28cm);
                    \fi
                \fi
            }
        \fi
